...
The license key for the demo is `0000`. The dashboard opens automatically once
...
The dashboard is reached through the ingress at `http://dashboard.local`, so it
needs `minikube tunnel` running and `dashboard.local` in `/etc/hosts`.
`run_demo.sh` enables the ingress addon and prompts for both before launching.
...
- **The dashboard needs manual setup.** `minikube tunnel` must be running and
  `dashboard.local` must resolve, or the browser opens on a dead host.
